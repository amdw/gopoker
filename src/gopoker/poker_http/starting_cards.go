...
func StartingCards(w http.ResponseWriter, req *http.Request) {
	...
	fmt.Fprintln(w, `return "in progress (" + $scope.resultsPending + " of " + $scope.requestsMade + " requests pending)"; }`)
	...
	fmt.Fprintln(w, `$scope.resultsPending -= 1;`)
	fmt.Fprintln(w, `}`)
	fmt.Fprintln(w, `$scope.onError = function (rank1, rank2, samesuit, error, status) { console.log("Got error for " + rank1 + rank2 + samesuit + ": " + status + "\n" + JSON.stringify(error)) }`)
	fmt.Fprintln(w, `});`)
	fmt.Fprintln(w, `</script>`)
