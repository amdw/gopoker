...
func playHoldem(w http.ResponseWriter, req *http.Request) {
	...
	pack := poker.NewPack()
	pack.Shuffle()
	onTable, playerCards, sortedHands := pack.PlayHoldem(players)
	fmt.Fprintf(w, "<h2>Table cards</h2><p>%v</p>", onTable)
	fmt.Fprintf(w, "<h2>Player cards</h2><ul>")
	for player := 0; player < players; player++ {
		fmt.Fprintf(w, "<li>Player %v: %v</li>", player+1, playerCards[player])
	}
	fmt.Fprintf(w, "</ul>")
	fmt.Fprintf(w, "<h2>Results</h2><table><tr><th>Position</th><th>Player</th><th>Hand</th></tr>")
	for i, hand := range sortedHands.Hands {
		fmt.Fprintf(w, "<tr><td>%v</td><td>%v</td><td>%v</td></tr>", i+1, sortedHands.Players[i], hand)
	}
	fmt.Fprintf(w, "</table>")
	fmt.Fprintf(w, "</body></html>")
}
